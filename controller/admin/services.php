...
$titleAdmin = "Servisler";
  ...
  elseif( $action == "new-service" ):
      if( $_POST ):
        ...
        elseif( $min > $max ):
          $error    = 1;
          $errorText= "The minimum order quantity cannot exceed the maximum order quantity.";
          $icon     = "error";
  ...
  elseif( $action == "new-subscription" ):
      if( $_POST ):
        ...
        elseif(  ( $package == 14 || $package == 15 ) && !is_numeric($autopost) ):
          $error    = 1;
          $errorText= "Shipment quantity cannot be empty";
          $icon     = "error";
        ...
        elseif(  ( $package == 14 || $package == 15 ) && !is_numeric($autotime) ):
          $error    = 1;
          $errorText= "Packet Term cannot be empty";
          $icon     = "error";
        else:
            $api=$conn->prepare("SELECT * FROM service_api WHERE id=:id "); $api->execute(array("id"=>$provider)); $api=$api->fetch(PDO::FETCH_ASSOC);
            if( $mode == 1 ): $provider = 0; $service = 0; endif;
  ...
  elseif( $action == "multi-action" ):
    ...
    $action   = $_POST["bulkStatus"];
      ...
      elseif( $action ==  "del_price" ):
        ...
        $insert->execute(array("c_id"=>$user["client_id"],"action"=>"<strong>Service deletion</strong> All authorizations have been taken for the transaction.","date"=>date("Y-m-d H:i:s"),"ip"=>GetIP() ));
                 ...
                 guardLogout();
                    ...
                    $insert = $conn->prepare("INSERT INTO guard_log SET client_id=:c_id, action=:action, date=:date, ip=:ip ");
        $insert->execute(array("c_id"=>$user["client_id"],"action"=>"<strong>Service deletion</strong> The session was terminated because the operation was completed.","date"=>date("Y-m-d H:i:s"),"ip"=>GetIP() ));
